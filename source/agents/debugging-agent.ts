...
function createWebSocketMessage(
  ...
  const confState: DebuggingAgentWebSocketConfigurationState = {
    ...
    connections: _.map(
      ...
      (conn) => ({
        source: [conn.source.sourceComponent.name, conn.source.sourcePort.name],
        target: [conn.target.targetComponent.name, conn.target.targetPort.name],
      })
    ),
  };
    ...
    switch (msg.ts_type) {
      ...
      case "DisconnectUnitMessage":
        processOrRelayMessage(
          ...
          msg.unit === runConfig.toExecute,
          () => {
            // disconnect unit by shutting down all ports
