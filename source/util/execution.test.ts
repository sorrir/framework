...
import { getDebuggingAgentAndConnections } from "../agents/debugging-agent";
...
describe("R-PM.3a_3b: multiple 1-1 connections", () => {
  ...
  const comp_3 = { ...comp_2, name: "comp_3" };
  const comp_4 = {
    ...comp_1,
    name: "comp_4",
  ...
  describe("R-PM.3a_3b: multiple 1-1 outgoing connections via REST", () => {
    ...
    test("debugging agent communication", async () => {
      ...
      const runConfigU2 = cloneRunConfig(sampleRunConfig, "u2");
      const runConfigU3 = cloneRunConfig(sampleRunConfig, "u3");
      restrictConfigStateToExecutedUnit(runConfigU1);
      restrictConfigStateToExecutedUnit(runConfigU2);
      restrictConfigStateToExecutedUnit(runConfigU3);

      // remove event out of component comp_1 as it is put into the event queue by the debugging server via websocket
      const eventQueue =
        runConfigU1.confState.componentState.get(comp_1)?.events;
      if (eventQueue) {
        eventQueue.length = 0;
      }
      ...
      (runConfigU1 as any).debuggingConfiguration = debuggingConfiguration;
      (runConfigU2 as any).debuggingConfiguration = debuggingConfiguration;
      (runConfigU3 as any).debuggingConfiguration = debuggingConfiguration;

      const dbgRunConfigU1 = setupAgents(runConfigU1);
      const dbgRunConfigU2 = setupAgents(runConfigU2);
      const dbgRunConfigU3 = setupAgents(runConfigU3);

      // start websocket client

      const ws = new WebSocket(
        `ws://${runConfigU1.hostConfiguration.u1.host}:${debuggingConfiguration.u1.debuggingAgent.webSocketPort}`
      );
