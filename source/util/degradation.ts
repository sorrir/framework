  ...
  components.forEach(function (
    ...
    if (state !== undefined) {
      const degradableState = {
        ...
        degradationHistory:
          state.degradationHistory !== undefined
...
function upgrade<S, E, P, D>(
  ...
  if (
    findPath(
      ...
      currentState.operatingMode,
      currentState.degradationHistory[0][0],
      []
    ).length === 0
  ) {
    console.log("No suitable path has been found.");
    return currentState;
  }
  // select all Transferfunctions that return a state in the target degradation mode
...
export function updateOperatingMode<S, E, P, D, SM>(
  ...
  const degradationLevel = [
    ...(component.dependencyMap?.keys() ? component.dependencyMap?.keys() : []),
  ]
    .sort()
    .reverse();
  for (const level of degradationLevel) {
    const dependency = component.dependencyMap?.get(level);
    if (dependency && dependency(currentState, shadows)) {
      if (level > currentState.operatingMode) {
        const newState = upgrade(component, currentState, level);
        if (newState !== currentState) return newState;
      } else if (level < currentState.operatingMode) {
        const newState = degrade(component, currentState, level);
        if (newState !== currentState) return newState;
      } else {
        return currentState;
      }
